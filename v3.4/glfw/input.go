...
func goPreeditCB(
	...
	preeditCount C.int,
	preeditString unsafe.Pointer,
	blockCount C.int,
	blockSizes unsafe.Pointer,
	focusedBlock C.int,
	caret C.int,
) {
...
// PreeditCallback sets the refresh callback of the window, which
...
// ImeStatusCallback is the window refresh callback.
type ImeStatusCallback func(w *Window)

// ImeStatusCallback sets the refresh callback of the window, which
// is called when the client area of the window needs to be redrawn, for example
// if the window has been exposed after having been covered by another window.
//
// On compositing window systems such as Aero, Compiz or Aqua, where the window
